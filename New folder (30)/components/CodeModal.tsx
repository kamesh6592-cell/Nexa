...
export const CodeModal: React.FC<CodeModalProps> = ({ data, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-brand-charcoal/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div 
        className="bg-brand-cream rounded-3xl shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col animate-[popIn_0.3s_ease-out_forwards]" 
        onClick={e => e.stopPropagation()}
      >
        <header className="flex justify-between items-center p-4 border-b border-brand-charcoal/10 flex-shrink-0">
          <h3 className="text-xl font-bold text-brand-charcoal">Generated Website Code</h3>
          <div className="flex items-center gap-2">
            <button 
                onClick={handleCopy}
                className="bg-white hover:bg-brand-sky text-brand-charcoal font-semibold py-2 px-4 rounded-full text-sm transition-all shadow-sm flex items-center gap-2"
            >
                {copyStatus === 'copied' ? (
            ...
            <button onClick={onClose} className="text-brand-charcoal/50 hover:text-brand-coral p-2 rounded-full">&times;</button>
          </div>
        </header>
