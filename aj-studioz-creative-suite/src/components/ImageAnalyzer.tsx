  ...
  const ToolButton: React.FC<{
    ...
    <button
      ...
      className={`p-2 rounded-lg flex items-center gap-2 transition-colors text-sm ${
        activeTool === tool ? 'bg-brand-coral text-white' : 'bg-brand-sky dark:bg-gray-600 dark:hover:bg-gray-500 text-brand-charcoal dark:text-white hover:bg-brand-peach'
      }`}
      aria-label={label}
    >
      {icon}
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="text-center mb-12">
        <h2 className="text-4xl md:text-5xl font-extrabold text-brand-charcoal dark:text-brand-cream">
          AI Homework Helper
        </h2>
        <p className="mt-4 text-lg text-brand-charcoal/70 dark:text-brand-cream/70">
          Snap a picture of any problem—even handwritten notes—and get the answers you need.
        </p>
      </div>

      <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-lg shadow-brand-peach/50 border border-brand-charcoal/5 dark:border-gray-700 space-y-6">
        <div>
            <label className="block text-md font-semibold text-brand-charcoal dark:text-brand-cream mb-2">
                1. Upload a picture of your problem
            </label>
            <p className="text-sm text-brand-charcoal/60 dark:text-brand-cream/60 mb-3 flex items-center gap-2">
             ...
             <div className="flex items-center space-x-4">
                 <label htmlFor="image-upload" className="cursor-pointer bg-brand-sky dark:bg-gray-600 dark:hover:bg-gray-500 text-brand-charcoal dark:text-white font-bold py-2 px-5 rounded-full transition-colors inline-block">Choose File</label>
                 <input id="image-upload" type="file" accept="image/png, image/jpeg, image/webp" onChange={handleFileChange} className="hidden" />
                 {imageFile && <span className="text-sm text-brand-charcoal/60 dark:text-brand-cream/60 truncate">{imageFile.file.name}</span>}
             </div>
        </div>
        ...
        {imageFile && (
          <div className="space-y-6 animate-[popIn_0.5s_ease-out_forwards]">
            <div>
                ...
                <div className="flex items-center gap-2 p-2 bg-brand-cream dark:bg-gray-700 rounded-xl mb-2 flex-wrap">
                    ...
                    <button onClick={handleUndo} className="p-2 rounded-lg bg-brand-sky dark:bg-gray-600 dark:hover:bg-gray-500 text-brand-charcoal dark:text-white hover:bg-brand-peach" aria-label="Undo"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clipRule="evenodd" /></svg></button>
                    <button onClick={handleClear} className="p-2 rounded-lg bg-brand-sky dark:bg-gray-600 dark:hover:bg-gray-500 text-brand-charcoal dark:text-white hover:bg-brand-peach" aria-label="Clear"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg></button>
                </div>
                <canvas 
                    ref={canvasRef} 
                    width="600" 
                    height="400" 
                    className="w-full h-auto bg-gray-200 dark:bg-gray-900 rounded-2xl cursor-crosshair"
            ...
            <div>
                ...
                <textarea
                    ...
                    rows={3}
                    className="w-full bg-brand-cream/70 dark:bg-gray-700 dark:text-white border-2 border-transparent rounded-xl p-3 focus:ring-2 focus:ring-brand-coral"
      ...
      <div className="mt-8">
        {isLoading && (
          <div className="flex flex-col items-center justify-center h-48">
            <LoadingSpinner />
            <p className="text-xl mt-4 text-brand-charcoal/70 dark:text-brand-cream/70">
        ...
        {analysisResult && !isLoading && imageFile && (
          <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-lg shadow-brand-peach/50 border border-brand-charcoal/5 dark:border-gray-700 space-y-6">
            <div className="flex justify-between items-center flex-wrap gap-4">
                <h3 className="text-2xl font-bold text-brand-charcoal dark:text-brand-cream">Here's the Answer!</h3>
                <button 
                  onClick={handleSharePdf}
                  className="bg-brand-sky dark:bg-gray-600 dark:hover:bg-gray-500 text-brand-charcoal/80 dark:text-white font-semibold py-1.5 px-4 rounded-full text-sm transition-all flex items-center gap-2"
                >
            ...
            <div className="space-y-6">
                 <div className="bg-brand-sky/50 dark:bg-gray-700/50 p-6 rounded-2xl">
                    <p className="font-semibold text-brand-charcoal dark:text-brand-cream mb-2 flex items-center gap-2">
                    ...
                    <blockquote className="text-sm text-brand-charcoal/70 dark:text-brand-cream/70 italic border-l-4 border-brand-peach pl-4 max-h-32 overflow-y-auto">
                 ...
                 <div className="bg-brand-sky/50 dark:bg-gray-700/50 p-6 rounded-2xl">
                    ...
                    <div className="markdown-content text-brand-charcoal/80 dark:text-brand-cream/80">
                      <ReactMarkdown remarkPlugins={[remarkGfm]}>{analysisResult.answer}</ReactMarkdown>
                    </div>
                </div>
            </div>
